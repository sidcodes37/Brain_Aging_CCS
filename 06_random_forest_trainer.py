...
OUT_DIR = "/serverdata/ccshome/sid/Final Pipeline/final_outputs/06_rf_outputs"
...
def objective(trial):
    ...
    # sklearn returns NEGATIVE MAE here, so negate it back to a positive MAE
...
pd.DataFrame([bp]).to_csv(os.path.join(OUT_DIR, "rf_best_params.csv"), index=False)
...
BA_tr = RF.predict(X_train)
BA_va = RF.predict(X_val)
BA_te = RF.predict(X_test)
...
shap = explainer(X_test, check_additivity=True) # If check_ddditivty is slow, then remove it. Else run it on a smaller subset.

# --- Save raw SHAP values ---
shap_cols = [f"shap_{c}" for c in feature_cols]
pd.DataFrame(shap.values, columns=shap_cols).to_csv(
    os.path.join(OUT_DIR, "shap_rf.csv"), index=False
)
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "shap_beeswarm_rf.png"), dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "shap_bar_rf.png"), dpi=150, bbox_inches='tight')
plt.show()
...
shap_summary.to_csv(os.path.join(OUT_DIR, "shap_summary_rf.csv"), index=False)
